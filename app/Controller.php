...
function GetForm() {
	...
	if (file_exists('../../app/form/'.$form_id.'.php')) {
		$form = include('../../app/form/'.$form_id.'.php');
	}
...
<div class="dot-files-body__file-btn-setting d-flex align-items-center justify-content-center" onclick=\'PageDotFileRemove("'.$time.'", "'.$_SERVER['DOCUMENT_ROOT'] . $path_file . $filename.'")\'>
...
<div class="dot-files-body__file-btn-plus d-flex align-items-center justify-content-center" onclick=\'PageDotFileChoose(this, "'.$path_file . $filename.'")\'>
...
function PageSave() {
	...
	if ($chunks['map']) {
		if (isset($chunks['map'][1])) {
			foreach ($chunks['map'][1] as $kl => $chunk) {
				ob_start();
				include('../chunks/'.$chunk.'.txt');
				$chunk_data = ob_get_clean();
				$html = str_replace($chunks['map'][0][$kl], $chunk_data, $html);
			}
		}
	}
	
	if ($chunks['script']) {
		if (isset($chunks['script'][1])) {
			foreach ($chunks['script'][1] as $kl => $chunk) {
				ob_start();
				include('../chunks/'.$chunk.'.txt');
				$chunk_data = ob_get_clean();
				$html = str_replace($chunks['script'][0][$kl], $chunk_data, $html);
			}
		}
	}*/
...
function GetHistory() {
	...
	foreach ($histories as $history) {
		...
		echo '
        <div class="pagedot-history-item">Сохранено - '.date('Y-m-d H:i', $last_update).'</div>
		';
	}
